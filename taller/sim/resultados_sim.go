...
func imprimirTablaResultados() {
	...
	fmt.Printf("%-15s %-12s %-15s %-20s %-10s %-10s %-10s %-10s\n",
		"Escenario", "Simulador", "Tiempo Total", "Tiempo Promedio Veh",
		"Entrada", "Atencion", "Limpieza", "Revision")
	for _, r := range resultados {
		fmt.Printf("%-15s %-12s %-15v %-20v %-10d %-10d %-10d %-10d\n",
			r.NombreEscenario,
			r.Simulador,
			r.TiempoTotal,
			r.TiempoMedioVeh,
